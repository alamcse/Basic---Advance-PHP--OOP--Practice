...
if(isset($_POST['form1'])) 
{
	...
	try {
		...
		if($total>0){
			$statement=$db->prepare("update sectors set  sector_description=? where sid=?");
			$statement->execute(array($_POST['sector_description'],$sid));
			
		}
		else{

		$statement=$db->prepare("update sectors set sector_name=?, sector_description=? where sid=?");
		$statement->execute(array($_POST['sector_name'],$_POST['sector_description'],$sid));
