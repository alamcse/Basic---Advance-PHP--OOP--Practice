...
if(isset($_POST['form1'])) 
{
	...
	try {
		...
		if($total>0){
			$statement=$db->prepare("update notice set  notice_description=? where nid=?");
			$statement->execute(array($_POST['notice_description'],$nid));
			
		}
		else{

		$statement=$db->prepare("update notice set notice_title=?, notice_description=? where nid=?");
		$statement->execute(array($_POST['notice_title'],$_POST['notice_description'],$nid));

	}
		$success_message='Notice has been successfully updated';
	...
	<div class="panel panel-default"style="margin-left:0px;">
		...
		<form method="post" action="">
			...
			<?php
				...
				<tr>
					<td><input type="text" name="notice_title" value="<?php echo $row['notice_title'];?>"></td>
				</tr>
				<tr>
				...
				<textarea class="ckeditor" id="editor" name="notice_description"cls="30" rows="10" >
